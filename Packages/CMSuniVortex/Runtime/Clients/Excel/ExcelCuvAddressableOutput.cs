...
{
    public abstract class ExcelCuvAddressableOutput<TModel, TModelList, TReference>
        : CuvAddressableOutput<TModel, TModelList, TReference>
        where TModel : ExcelModel
        where TModelList : ExcelCuvModelList<TModel>
        where TReference : ExcelCuvAddressableReference<TModel, TModelList>
    {
...
#if UNITY_EDITOR
            var buildFullPath =  GetReferencePath(buildPath);
            if (_reference == default && File.Exists(buildFullPath))
            {
                _reference = AssetDatabase.LoadAssetAtPath<TReference>(buildFullPath);
            }
#endif
