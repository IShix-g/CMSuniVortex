...
{
    public abstract class CockpitCuvAddressableOutput<TModel, TModelList, TReference> : CuvAddressableOutput<TModel, TModelList, TReference>
        ...
        where TReference : CockpitCuvAddressableReference<TModel, TModelList>
    {
        ...
        public override void Generate(string buildPath, ICuvClient client, string[] listGuids)
        {
            ...
            for (var i = 0; i < objs.Length; i++)
            {
                var obj = objs[i];
                var groupName = GetGroupName(obj.Language);
                AddressableHelper.AddTo(groupName, obj);
